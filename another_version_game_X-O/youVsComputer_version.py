...
class Player:
    def __init__(self, is_computer=False):
        self.name = " "
        self.symbol = " "
        self.is_computer = is_computer  # جديد
...
class Game:
    def __init__(self):
        self.board = Board()
        self.menu = Menu()
        self.players = [Player(), Player(is_computer=True)]  # اللاعب الثاني = كمبيوتر
    ...
    # It checks all possible winning conditions (rows, columns, diagonals)
